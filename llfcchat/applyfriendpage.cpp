    ...
    ui(new Ui::ApplyFriendPage)
{
    ui->setupUi(this);
    this->setStyleSheet(
        "QWidget#ApplyFriendPage{background:#f6f8fb;}"
        "QWidget#friend_apply_wid{background:#ffffff;border-bottom:1px solid #e4e9ee;}"
        "QLabel#friend_apply_lb{color:#17212b;font:600 20px 'Microsoft YaHei';padding-left:4px;}"
        "ApplyFriendList{background:#f8fafc;border:none;outline:none;padding:10px 0;}"
    );
    ...
    connect(TcpMgr::GetInstance().get(), &TcpMgr::sig_add_auth_friend, this,
            [this](std::shared_ptr<AuthInfo> auth_info) {
                auto find_iter = _unauth_items.find(auth_info->_uid);
