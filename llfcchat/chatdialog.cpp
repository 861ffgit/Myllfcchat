    ...
    _cur_chat_thread_id(0), _loading_dlg(nullptr), _cur_load_chat(nullptr)
{
    ...
    for (QWidget* widget : { ui->side_bar, ui->chat_user_wid, ui->search_wid }) {
        widget->setAttribute(Qt::WA_StyledBackground, true);
    }
    ui->chat_user_list->setSpacing(4);
    ui->con_user_list->setSpacing(4);
    ui->search_list->setSpacing(4);
    ui->side_bar->setStyleSheet("QWidget#side_bar{background:#2c3138;border-right:1px solid #252a30;}");
    ui->chat_user_wid->setStyleSheet("QWidget#chat_user_wid{background:#f3f5f7;border:none;}");
    ui->search_wid->setStyleSheet("QWidget#search_wid{background:#f0f3f6;border-bottom:1px solid #e3e8ed;}");
    ui->chat_user_list->setStyleSheet(
        "ChatUserList{background:#f3f5f7;border:none;outline:none;padding:8px 4px;}"
        "ChatUserList::item{margin:4px 8px;border-radius:8px;}"
        "ChatUserList::item:selected{background:#dde5e8;border:none;outline:none;}"
        "ChatUserList::item:hover{background:#e8edf1;border:none;outline:none;}"
    );

    ui->add_btn->SetState("normal", "hover", "press");
    ui->add_btn->setProperty("state", "normal");
    ui->add_btn->setIcon(QIcon(":/res/add_friend_normal.png"));
    ui->add_btn->setIconSize(QSize(24, 24));
    ui->add_btn->setStyleSheet(
        "ClickedBtn[state='normal']{background:#f7f8fa;border:1px solid #e4e7eb;border-radius:8px;border-image:none;padding:0px;}"
        "ClickedBtn[state='hover']{background:#ffffff;border:1px solid #d9dde2;border-radius:8px;border-image:none;padding:0px;}"
        "ClickedBtn[state='press']{background:#eef2f5;border:1px solid #d8dde3;border-radius:8px;border-image:none;padding:0px;}"
    );
    ui->add_btn->installEventFilter(this);
    QAction* searchAction = new QAction(ui->search_edit);
    searchAction->setIcon(QIcon(":/res/search.png"));
    ui->search_edit->addAction(searchAction, QLineEdit::LeadingPosition);
    ui->search_edit->setPlaceholderText(QString::fromUtf8(u8"搜索"));
...
{
    if (watched == ui->add_btn) {
        if (event->type() == QEvent::Enter) {
            ui->add_btn->setIcon(QIcon(":/res/add_friend_hover.png"));
...
void ChatDialog::SyncContactRedPoint()
{
    const bool has_pending_apply = UserMgr::GetInstance()->HasPendingApply();
    ui->side_contact_lb->ShowRedPoint(has_pending_apply);
    ui->con_user_list->ShowRedPoint(has_pending_apply);
}
...
void ChatDialog::slot_add_auth_friend(std::shared_ptr<AuthInfo> auth_info) {
    ...
    UserMgr::GetInstance()->UpdateApplyStatus(auth_info->_uid, 1);
    SyncContactRedPoint();
...
//todo: 点击搜索的联系人聊天，跳转到聊天界面，之后添加申请创建私聊或者查找已存在私聊消息
...
void ChatDialog::slot_update_upload_progress(std::shared_ptr<MsgInfo> msg_info) {
    ...
    //更新消息，其实不用更新，都是共享msg_info的一块内存，这里为了安全还是再次更新一次
