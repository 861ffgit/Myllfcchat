    ...
    ui(new Ui::RegisterDialog),_countdown(5)
{
    ...
    for (CustomizeEdit *edit : edits) {
        edit->setFrame(false);
        edit->setTextMargins(14, 0, 14, 0);
        edit->SetBackgroundColor(QColor(0xf6, 0xfb, 0xf7), QColor(0xff, 0xff, 0xff));
    }
    ...
    connect(_countdown_timer, &QTimer::timeout, [this](){
        ...
        auto str = tr("注册成功，%1 s 后返回登录").arg(_countdown);
...
void RegisterDialog::ChangeTipPage()
{
    _countdown_timer->stop();
    _countdown = 5;
    ui->tip_lb->setText(tr("注册成功，5 s 后返回登录"));
